...
import org.berusted.craftable.api.CraftableEvents;
...
final class DirectCraftingTransaction {
    ...
    private static void notifyCrafted(ServerPlayer player, DirectCraftingPlan plan) {
        try {
            // The Fabric callback replaces NeoForge's PlayerEvent.ItemCraftedEvent and
            // is fired from the same point: after the item sees the craft, before the
            // recipe-book and advancement notifications.
            ItemStack crafted = plan.output();
            crafted.onCraftedBy(player.serverLevel(), player, crafted.getCount());
            CraftableEvents.ITEM_CRAFTED.invoker().onItemCrafted(player, crafted, plan.gridItems());
