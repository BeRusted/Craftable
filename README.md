...
当前仓库已完成 M0.1、M0.2、M1、M1.1 和 M2。M2 的自动验证、首轮人工测试及轮播目标定向复测均已通过：一次普通原版 2×2/3×3 配方会在服务端重新扫描、精确规划、复验并补偿式提交，支持配方剩余物、背包容量预检、组件指纹和原版/NeoForge 制作通知。下一步是 M3 原版配方书增强，包括可见页主动状态、`showing craftable` 统一筛选、无闪烁状态刷新与实体 3×3 背包菜单。目标版本为 Minecraft `1.21.1`、NeoForge `21.1.235`、Java 21。
