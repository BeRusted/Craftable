...
当前 M0–M3 已验收；M3 于 2026-09-08 完成两机人工复测与收口，非阻断观察项见验收报告。M3 加入原版配方书主动状态、统一“仅可合成”筛选、不污染解锁进度的完整普通合成目录，以及附近工作台提供的真实 3×3 背包菜单。`C` 仍复用 M2 服务端单步事务；递归、炉类、撤销和第三方兼容尚未实现。目标版本为 Minecraft `1.21.1`、NeoForge `21.1.235`、Java 21。

仅生存/冒险模式在附近有工作台时扩展背包为 3×3；复用原版背包纹理、人物渲染及装备栏位置。创造/旁观无 Craftable 菜单、按钮、预取或 C 操作。背包与实体工作台配方书均增强。左键配方放置沿用原版，只从随身材料填格；使用附近容器材料请按 `C`。原版“选项”页右上角的 Craftable 按钮直达已有配置页。多人测试双方必须使用同一份新 JAR（协议版本 5）。
