...
- JEI 存在且启用时，内置主物品/配方浏览器自动隐藏；
- Craftable 的环境状态、计划预览、失败原因仍保留；
...
第二阶段再做用途查询、收藏、历史、标签高级搜索、复杂第三方类别布局。内置浏览器与 JEI 共享目标选择控制器，但渲染层互不依赖。
...
| 键 | 建议默认 | 说明 |
|---|---:|---|
| `browserMode` | `AUTO` | `AUTO/ALWAYS/NEVER`，AUTO 遇到 JEI 隐藏 |
...
- `Open Ambient Browser`：默认不绑定或选择无冲突键；
